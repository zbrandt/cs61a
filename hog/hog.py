...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    first, tens = player_score % 10, (opponent_score // 10) % 10
    brawl = 3 * abs(first - tens)
    if brawl > 1:
        return brawl
    else:
        return 1
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    score, scorest, highest, roll = 0, 0, 0, 1
    while roll <= 10:
        score = make_averaged(roll_dice, samples_count)(roll, dice)
        if score > scorest:
            scorest = score
            highest = roll
        roll += 1
