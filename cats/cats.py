...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    new = [para for para in paragraphs if select(para)]
    if k <= len(new) - 1:
        return new[k]
    else:
        return ''
...
def about(subject):
    ...
    def contains(paragraph):
        ...
        return len([True for sub in subject if [True for word in words if sub == word]]) > 0
    return contains
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if source == '':
        return 0.0
    accuracy = 0.0
    error_metric = len(typed_words)

    if len(typed_words) > len(source_words):
        nums = list(range(len(source_words)))
    else:
        nums = list(range(len(typed_words)))

    for i in nums:
        if typed_words[i] == source_words[i]:
            accuracy += 100 / error_metric
    return accuracy
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    minimum = min(word_list, key = helper)
    if diff_function(typed_word, minimum, limit) > limit:
        return typed_word
    else:
        return minimum
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit == 0:
        return limit + 1
    while len(typed) and len(source) and typed[0] == source[0]:
        typed, source = typed[1:], source[1:]
    if not len(typed) and len(source):
        return len(source)
    if not len(source) and len(typed):
        return len(typed)
    else:
...
def report_progress(typed, source, user_id, upload):
    ...
    """
    # BEGIN PROBLEM 8
    progress, error = 0, False
    for i in range(len(typed)):
        if typed[i] != source[i]:
            error = True
        if not error and typed[i] == source[i]:
            progress += 1
...
def time_per_word(words, timestamps_per_player):
    ...
    for times in timestamps_per_player:
        ...
        for i in range(len(times) - 1):
            differences += [times[i + 1] - times[i]]
        amounts += [differences]
    return match(words, amounts)
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if get_all_times(match)[j][i] < get_all_times(match)[fastest][i]:
                fastest = j
        sayings[fastest] += [get_all_words(match)[i]]
    return sayings
    # END PROBLEM 10
