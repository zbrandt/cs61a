...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        # print('DEBUG:', expressions.rest.map(lambda x: scheme_eval(x, env)).first)
        env.define(expressions.first, expressions.rest.map(lambda x: scheme_eval(x, env)).first)
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            ...
            if clause.rest == nil:
                ...
                if clause.rest.rest == nil:
                    return scheme_eval(clause.rest.first, env)
                else:
                    scheme_eval(clause.rest.first, env)
                    return eval_all(clause.rest.rest, env)
            # END PROBLEM 13
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        names, vals = Pair(name, names), Pair(scheme_eval(val, env), vals)
