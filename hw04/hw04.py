...
def balanced(m):
    ...
    if is_planet(end(left(m))):
        left_torque = mass(end(left(m))) * length(left(m))
    if is_planet(end(right(m))):
        right_torque = mass(end(right(m))) * length(right(m))
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        return label(t)
    [sums.append(label(t) + max_path_sum(b)) for b in branches(t) if is_tree(b)]
    return max(sums)
